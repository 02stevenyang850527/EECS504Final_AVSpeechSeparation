...
if __name__ == '__main__':
   ...
   raw_path = '/home/ubuntu/eecs504/obamanet/data/videos/'
   save_path = '/home/ubuntu/eecs504/multisensory/data/obamanet/train/'
   ...
   d = 224
   for f in tqdm(files):
      with VidFrames(vid_path=f, sound = True, dims=(256,256), fps = 29.97) \
                     as (im_files, snd_file):

         clip_point = list(range(10,120, 5)) # start from 10 sec, ends at 115 secs, step = 5 sec
         ...
         length = snd.length
         assert(length > 120), f"length of video ({lengrh} sec) should be greater than 120"

         for x in clip_point:
            im = ims[int(x*fps) : int((x+5)*fps)]
            assert (im.shape[0] >= sampled_frames), f"Video samples {im.shape[0]} is smaller than the {sampled_frames} vid_dur = {vid_dur}!"
            im = im[:sampled_frames]
            np.save(save_path+ f'frames/{cnt}.npy', im)
            snd_clip = snd_copy[int(round(samples_per_frame*int(x*fps))):int(round(samples_per_frame*int((x+5)*fps)))]
            assert (snd_clip.shape[0] >= num_samples), f"Audio samples {snd_clip.shape[0]} is smaller than the {num_samples} vid_dur = {vid_dur}!"
            np.save(save_path+ f'audios/{cnt}.npy', snd_clip[:num_samples])
            if (cnt > 2600):
               break;
            else:
               cnt += 1
